...
class TeacherModel(nn.Module):
    ...
    def __init__(self, image_size, in_channel, num_blocks, channels, num_classes=7, **kwargs):
        ...
        # stem block = single convolution block followed by batchNorm and non-linear activation
        self.stage1 = self._make_block(block['stem'],in_channel,channels[0],3,num_blocks[0])
        # consists of multiple blocks of MBCSA, essentially instead of conv of in*out*channels, it is 1*1*channels more and then depth wise i.e. each channel individually convolution and then 1*1*out_channels convolution , standard is in*out*in_channels size of out_channels
    ...
    def _make_block(self,block,inp,oup,kernel_size,depth):
        ...
        for i in range(depth):
            if i==0:
                # only first layer downsamples, otherwise it is always same size
