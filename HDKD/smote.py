...
def map_classes_to_labels(classes_distribution, class_mapping):
    ...
    for _class, val in classes_distribution.items():
        # mapping is subfolder sorted and assigned index
        dct[class_mapping[_class]] = int(val)
    #dct is index assigned to that class and it's integer value
    return dct
...
def apply_smote(args,train_transforms):
    try:
        ...
        # ibmlearn library for smote 
        # this will return the mentioned number of entites 
        smote = SMOTE()
