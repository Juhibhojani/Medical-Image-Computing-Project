...
def main(args):
    ...
    # adding class distirbution in case of using smote
    if args.use_smote:
        with open("classes_distribution.json", "r") as file:
            args.classes_distribution = json.load(file)

    print("")
    # building dataset, for both train and test
    ...
    # timm library is being used here
    # models are defined in libraries
    model = create_model(
        args.model,
    ...
    # loading the already pretrained model
    # TODO: dataset path given here
    model.load_state_dict(torch.load("/home/micml/25m0764/MIC_project/HDKD/weighted_loss_data_models/teacher_model/teacher_model_weighted_loss.pth"))
    ...
    if args.model=="teacher_model":
        ...
        # optimizer is set with lower learning rate that original
    ...
    # freezing batch normalization parameters as well
    def freeze_bn(module):
        if isinstance(module, nn.BatchNorm2d):
            module.eval()

    model.stage1.apply(freeze_bn)
    model.stage2.apply(freeze_bn)

    print("-"*100)
    print("Validating how parameters which are freezed and which aren't")
    for name, param in model.named_parameters():
        print(name, param.requires_grad)

    # printing total number of parameters - unfreezed
    print("-"*100)
    num_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('The {} has number of unfreezed params = {}'.format(args.model,num_parameters))
    print("-"*100)   
